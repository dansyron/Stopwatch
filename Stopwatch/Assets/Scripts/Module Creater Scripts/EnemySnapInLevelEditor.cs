...
/// <summary>
/// Allows the enemy to be snapped into the correct location in the level editor
/// </summary>
public class EnemySnapInLevelEditor : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (prefabSpawn.switchToLevelDesigner)
        {
            ...
            else if (tempRoundedPosition.y < transform.position.y && tempRoundedPosition.x < transform.position.x)
            {
                ...
                transform.position = (new Vector3(Mathf.Round(transform.position.x) + (componentOffset / 2), Mathf.Round(transform.position.y), 0));
            }
